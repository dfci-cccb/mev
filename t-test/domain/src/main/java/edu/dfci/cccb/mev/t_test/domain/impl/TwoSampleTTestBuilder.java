...
import edu.dfci.cccb.mev.t_test.domain.contract.InvalidTwoSampleTTestConfigurationException;
...
@Log4j
public class TwoSampleTTestBuilder extends AbstractTTestBuilder {
  ...
  public TTest build () throws DatasetException {
    ...
    if (dimension == null)
      throw new InvalidTwoSampleTTestConfigurationException ();
    
    
    try {
      TemporaryFolder tempTTestFolder = new TemporaryFolder ();

      log.debug ("Using " + tempTTestFolder.getAbsolutePath () + " for two-sample t-test analysis");
      ...
      try {
        ...
        //write the configuration file.  Since this is for the two-sample test, we only have two "groupings" of samples
        ...
        try (PrintStream configOut = new PrintStream (new FileOutputStream (configFile))) {
          for (int index = 0; index < dimension.keys ().size (); index++){
            if (controlSelection ().keys ().contains (dimension.keys ().get (index))
                    && experimentSelection ().keys ().contains (dimension.keys ().get (index)))
                  throw new InvalidTwoSampleTTestConfigurationException ();
                else if (experimentSelection ().keys ().contains (dimension.keys ().get (index)))
                  configOut.println (dimension.keys ().get (index) + "\t1\tNA");
                else if (controlSelection ().keys ().contains (dimension.keys ().get (index)))
        ...
        try (ByteArrayOutputStream script = new ByteArrayOutputStream ();
          ...
          if(equalVarianceFlag ()){
            ...
            printScript.println (EQUAL_VARIANCE+"=FALSE");
          }
